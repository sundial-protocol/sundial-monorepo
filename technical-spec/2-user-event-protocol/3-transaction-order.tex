  ...
  \item[Authenticate Order.] Properly record a new L2 transaction order order event on L1.
    Conditions:
      \begin{enumerate}
        ...
        \item Validity of the order must be set to \code{TxIsValid}.
      \end{enumerate}
...
\begin{description}
  \item[Spend.] Transaction for spending a confirmed transaction order.
    ...
    \begin{enumerate}
      ...
      \item Let \code{validity\_override} be the expected validity of the transaction order stored in transactions tree of \code{settlement}, provided via the redeemer.
      \item The transaction order must be included in the transactions tree of \code{settlement} with its validity set to \code{validity\_override}.
