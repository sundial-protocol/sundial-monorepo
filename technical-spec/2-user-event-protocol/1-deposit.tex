...
\begin{enumerate}
    ...
    \item If any Midgard-compatible tokens are included in the deposit, they must be recorded in \code{DepositEvent}, and withheld within the utxo until maturity. However, all these tokens must be burnt when transferring deposits to the Midgard reserve.
    \item Total count of tokens (i.e. Midgard-compatible tokens and tokens that don't have the mint/burn logic for cross-layer transfers) must not exceed \code{max\_transfer\_token\_count}, a Midgard protocol pararmeter.
  ...
  \item[Authenticate Deposit.] Properly record a new deposit event from L1 to L2.
    Conditions:
      \begin{enumerate}
        ...
        \item Total number of tokens included in the deposit utxo must not exceed \code{max\_transfer\_token\_count}.
...
\begin{description}
  \item[Transfer to Reserve.] Transaction for moving funds of finalized deposit events into Midgard reserve.
    ...
    \begin{enumerate}
      ...
      \item Inclusion time of the deposit must fall within the time interval of \code{settlement\_node}.
...
The \code{witness} script is parameterized by transaction ID of the resulting L2 output reference from a deposit, which obtained by hashing the serialized L1 output reference with Blake2b256.
The registration state of this script is the mechanism that allows related fraud proofs to function.
Conditions:

\begin{enumerate}
  \item Let \code{l2\_id} be the Blake2b256 hash of deposit event's \code{deposit\_id} (L1 nonce output reference).
  \item Registration/unregsitration of this witness script is respectively tied to the \code{deposit} minting policy's mint/burn logic. 
    \begin{itemize}
      \item Registeration is allowed if a deposit NFT with asset name of \code{l2\_id} is minted.
      \item Unregistration is allowed if a deposit NFT with asset name of \code{l2\_id} is burnt.
    \end{itemize}
\end{enumerate}
