  ...
  \item[Authenticate Deposit.] Properly record a new deposit event from L1 to L2.
    ...
    Conditions:
      \begin{enumerate}
        ...
        \item The redeemer used for unregistrering the witness script must be correct. Namely, it must be equal to \code{deposit} policy ID.
      \end{enumerate}
\end{description}
...
\begin{description}
  \item[Transfer to Reserve.] Transaction for moving funds of finalized deposit events into Midgard reserve.
    ...
    \begin{enumerate}
      ...
      \item The \code{BurnDepositNFT} endpoint of the deposit minting script must be invoked with the corresponding \code{l2\_id} of the deposit utxo.
...
The \code{witness} script is parameterized by transaction ID of the resulting L2 output reference from a deposit, which is obtained by hashing the serialized L1 output reference with Blake2b256.
...
\begin{enumerate}
  ...
  \item Let \code{deposit\_minting\_policy} be the policy ID of the deposit NFT, accessed via the redeemer.
  \item Registration/unregsitration of this witness script is respectively tied to the \code{deposit\_minting\_policy}'s mint/burn logic. 
    \begin{itemize}
