...
\begin{description}
    \item[Init.] Conditions:
        \begin{enumerate}
            ...
            \item The \code{output\_state} datum type must be \code{StepDatum(Data)}, with its \code{data} being \code{None} (meaning there should be no data in the initial state).
    ...
    \item[Success.] Terminate the state machine normally from the final spending validator in the computation.
      Couples its validation with that of the fraud proof minting policy.
      Conditions:
        \begin{enumerate}
            \item Let \code{fraud\_proof} be the policy ID of the fraud proof token, and \code{computation\_thread}, retrieved from the hub oracle utxo.
            \item Let \code{successful\_computation\_thread\_token} be the asset name of the computation thread that has successfully completed, provided via the redeemer.
            \item The transaction must burn the computation thread token with asset name \code{successful\_computation\_thread\_token}.
            \item The transaction must mint the fraud proof token defined by the fraud proof minting policy.
            \item The transaction must \emph{not} mint or burn any other tokens.
        \end{enumerate}
    \item[BurnForCancellation.] Terminate the state machine exceptionally from any spending validator in the computation.
      Conditions:
        \begin{enumerate}
            \item Let \code{burning\_computation\_thread\_token} be the asset name of the computation thread being canceled, provided via the redeemer.
            \item The transaction must burn the computation thread token with asset name \code{burning\_computation\_thread\_token}.
            \item The transaction must \emph{not} mint or burn any other tokens.
        \end{enumerate}
\end{description}
...
\begin{description}
            ...
            \item If this is the last step of the computation:
                \begin{itemize}
                    \item Mint the fraud token, which will implicitly burn the computation thread token with the Success redeemer.
                      Let \code{output\_state} be that transaction output.
                    \item The \code{output\_state} datum type must be \code{StepDatum(Data)}, with its \code{data} being \code{None}.
    ...
    \item[Cancel.] Cancel the computation.
      Conditions:
        \begin{enumerate}
            \item Validate the `Cancel` redeemer from the computation thread policy is invoked (this in turn validates the burning of the specified computation thread token).
            \item The transaction must be signed by the fraud prover pub-key defined in the input datum.
            \item The transaction must \emph{not} mint or burn any other tokens.
        \end{enumerate}
\end{description}
