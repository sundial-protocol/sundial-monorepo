...
The bond unlock time in the operator's node in the Operator Directory should be advanced whenever an operator optimistically marks a settlement as resolved, similar to how it is updated when committing a block to the state queue.
...
If the operator already has a later bond unlock time from another unresolved obligation, that later timestamp must be preserved.
  ...
  \item[Spawn.] Create a settlement utxo to store a merged block's deposits, withdrawals and transactions.
    Conditions:
      \begin{enumerate}
        ...
        \item The transaction must Fold a \code{state\_queue} node via the Merge To Confirmed State redeemer.
  ...
  \item[Attach Resolution Claim.] The current operator attaches a resolution claim to a settlement utxo.
    Conditions:
      \begin{enumerate}
        ...
        \item The Update Bond Hold New Settlement redeemer must provide a \code{resolution\_time} value.
        \item The \code{resolution\_time} of the attached resolution claim must match this redeemer value.
          The active operators validator ensures this value equals the current claim's maturity time and that the operator node's \code{bond\_unlock\_time} is advanced to at least this value without shortening any previous hold.
