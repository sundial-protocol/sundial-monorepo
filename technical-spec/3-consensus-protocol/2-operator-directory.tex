...
\begin{description}
    ...
    \item[Activate Operator.] Transfer an operator node from the \code{registered\_operators} queue to the \code{active\_operators} set.
      Conditions:
        ...
        \end{enumerate}
    \item[Remove Operator Bad State.] Remove an operator's node from the \code{active\_operators} set without returning the operator's ADA bond to the operator.
      Conditions:
        \begin{enumerate}
