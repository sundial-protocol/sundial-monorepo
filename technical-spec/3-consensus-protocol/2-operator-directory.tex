...
\begin{align*}
    \T{RegisteredOperator} &\coloneq \Bigl( \T{operator}: \T{PubKeyHash} \Bigr) \\
...
\begin{description}
    ...
    \item[Register Operator.] Insert a key-descending node into the \code{registered\_operators} queue with the operator's ADA bond, the registration time as key, and storing the operator's pub key hash.
      Grouped conditions:
        \begin{itemize}
            \item Verify the registration and key-descending insertion of the registrant to the registered operators:
            \begin{enumerate}
                \item The transaction must Insert a node into the \code{registered\_operators} queue.
                  Let that node be \code{registered\_node}.
                \item Anchor element of \code{registered\_node} must be root.
                ...
                \item The key of \code{registered\_node} must be the big-endian encoding of \code{activation\_time} in as few bytes as possible.
                \item The \code{operator} field of \code{registered\_node} must correspond to a PKH that has signed the transaction.
      ...
      Grouped conditions:
        \begin{itemize}
            ...
            \begin{enumerate}
                ...
                \item The inclusive lower bound of the transaction validity interval must meet or exceed the \code{activation\_time}.
            \end{enumerate}
