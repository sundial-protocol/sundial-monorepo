...
\Cref{fig:block-transition} shows how the block's utxo set is derived by applying the block's withdrawals, transactions, and then deposits to the previous block's utxo set.
\Cref{fig:block-tx-mpt} shows an example MPT for block transactions.
...
\begin{itemize}
    ...
    \item The \code{prev} field is a hash of the previous block header.
...
The block header's \code{tx\_root} is shown at the top in orange.}
