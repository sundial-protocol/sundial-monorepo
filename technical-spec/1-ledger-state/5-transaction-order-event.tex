...
In turn, the operator is free to set \code{MidgardTxValidity} as needed, if the included transaction is invalid.
...
The \code{TxOrderId} of a transaction order event corresponds to one of the inputs spent by the user in the L1 transaction that created the L1 order utxo (more specifically, it's the hash of its serialized output-reference).
