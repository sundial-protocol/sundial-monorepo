...
\notebox{Midgard's transaction types modify the simplified types above mainly by adding counter fields for variable-length fields, along with other changes to facilitate L1 scripts traversing the L2 transaction contents to verify fraud proofs.}

\notebox{
    The DA layer will be responsible for confirming hashes of lists, sets or other objects.
    In other words, the mismatched hashes are a form of DA frauds.
}
