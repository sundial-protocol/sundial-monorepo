...
module Testing.MembershipValidator (tests, dumpDebug) where
...
tests =
  testGroup
    "Membership Validator Tests"
    [ testCase "Simple" $
        psucceeds $
          membershipStakeValidator # pconstant globalLogicScriptContext
    ]
...
authCS = CurrencySymbol "deadbeef"
...
midgardValidatorTxOut key next commitment =
  TxOut
    ...
    , txOutValue =
        sortValue
          ( Value.singleton adaSymbol adaToken 2_000_000
  ...
  where
    midgardNodeCS = CurrencySymbol "deadbeef"
