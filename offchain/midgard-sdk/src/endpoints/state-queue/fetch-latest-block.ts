...
const getLinkFromBlockUTxO = (blockUTxO: UTxO): Either.Either<NodeKey, string> => {
...
export const fetchLatestBlock = (
  ...
  config: Config
): Effect.Effect<[UTxO, NodeKey], string> =>
  Effect.gen(function* () {
    ...
    const blockNFTsEithers: Either.Either<[UTxO, NodeKey], string>[] = allBlocks.map((u: UTxO) => {
      const nfts = getSingleAssetApartFromAda(u.assets);
      return Either.andThen(nfts, ([sym, _tn, qty]): Either.Either<[UTxO, NodeKey], string> => {
    ...
    const authenticBlockUTxOs = yield* Effect.filterMap(blockNFTsEithers, Option.some);
