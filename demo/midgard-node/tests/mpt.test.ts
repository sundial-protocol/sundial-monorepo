...
describe("The makeMpts tests", () => {
  it.effect("Initialization", (_) =>
    Effect.gen(function* () {
      const config = yield* NodeConfig;
      yield* flushAll;
      yield* Effect.try({
        try: () =>
          FS.rmSync(config.MEMPOOL_MPT_DB_PATH, {
            recursive: true,
            force: true,
          }),
        catch: (e) =>
      ...
      yield* Effect.try({
        try: () =>
          FS.rmSync(config.LEDGER_MPT_DB_PATH, {
            recursive: true,
            force: true,
          }),
        catch: (e) =>
          new Error("Failed to delete ledger LevelDB file from disk"),
      });
      ...
      expect(txRootInit).toBe(toHex(mempoolTrie.EMPTY_TRIE_ROOT));
      expect(utxoRootInit).toBe(ledgerRootAfterGenesis);

      const { utxoRoot, txRoot } = yield* processMpts(ledgerTrie, mempoolTrie, [
        txEntry1,
      ]);

      expect(utxoRoot).not.toBe(toHex(ledgerTrie.EMPTY_TRIE_ROOT));
      expect(txRoot).not.toBe(toHex(ledgerTrie.EMPTY_TRIE_ROOT));
      expect(txRoot).not.toBe(ledgerRootAfterGenesis);
      ...
      yield* Effect.tryPromise({
        try: () => mempoolDB.close(),
        catch: (e) => new Error(`Failed to close mempoolDB: ${e}`),
      });
    }).pipe(
      Effect.provide(Database.layer),
      Effect.provide(Lucid.Default),
...
const ledgerRootAfterGenesis = "17f312496e751e4ba35054e5ee87ec0293be38172af1034c8724e4194f310dcb"
