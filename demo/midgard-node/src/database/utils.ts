...
export const mkKeyValueCreateQuery = (tableName: string): Effect.Effect<void, Error, Database> =>
  Effect.gen(function* () {
...
export const delMultiple = (
  ...
  Effect.gen(function* () {
    const sql = yield* SqlClient.SqlClient;
    yield* Effect.logDebug(`${tableName} db: attempt to delete multiply entries`);
...
export const insertKeyValues = (
  ...
  Effect.tapErrorTag("SqlError", (e) =>
    Effect.logError(`${tableName} db: insert keyValues: ${JSON.stringify(e)}`),
  ),
  mapSqlError,
);
