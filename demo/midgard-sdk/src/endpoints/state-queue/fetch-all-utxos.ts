...
import { getSingleAssetApartFromAda, utxosAtByNFTPolicyId } from "@/utils/common.js";
import { makeReturn } from "@/core.js";

export const fetchAllAuthenticUTxOsProgram = (
  lucid: LucidEvolution,
  config: FetchConfig
): Effect.Effect<{utxo: UTxO, assetName: string}[], Error> => Effect.gen(function* () {
  const allUTxOs = yield* utxosAtByNFTPolicyId(
    lucid,
    config.stateQueueAddress,
    config.stateQueuePolicyId
  );
  return yield* Effect.allSuccesses(allUTxOs.map((u: UTxO) => {
 ...
 */
export const fetchAllAuthenticUTxOs = (
  lucid: LucidEvolution,
  config: FetchConfig
) => makeReturn(fetchAllAuthenticUTxOsProgram(lucid, config)).unsafeRun();
