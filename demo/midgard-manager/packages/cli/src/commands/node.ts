  ...
  ({ endpoint }) => {
    return pipe(
      Effect.tryPromise(async () => {
        const spinner = ora(
          `Checking Midgard node status at ${endpoint}...`
        ).start();
        ...
        try {
          ...
          console.log(
            chalk.white(`• Version: ${chalk.green(data.version || "unknown")}`)
          );
          console.log(
            chalk.white(`• Chain: ${chalk.green(data.network || "unknown")}`)
          );

          if (data.blockHeight) {
            console.log(
              chalk.white(`• Block Height: ${chalk.green(data.blockHeight)}`)
            );
          }
          ...
          if (data.peers) {
            console.log(
              chalk.white(
                `• Connected Peers: ${chalk.green(data.peers.length)}`
              )
            );
          }
          ...
          if (data.mempool) {
            console.log(
              chalk.white(
                `• Mempool Transactions: ${chalk.green(data.mempool.count)}`
              )
            );
          }
          ...
          console.log(
            chalk.gray(`  $ midgard-manager configure-node --endpoint <url>`)
          );
        ...
        try {
          ...
          try {
            ...
            if (response.ok) {
              ...
              console.log(
                chalk.yellow(
                  `⚠️ Node responded with status: ${response.status}`
                )
              );
            }
          } catch (error) {
            console.log(
              chalk.yellow(`⚠️ Could not connect to node: ${error.message}`)
            );
          }
