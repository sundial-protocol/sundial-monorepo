...
export const generateTxCommand = Command.make(
  "generate-tx",
  {
    ...
    concurrency: Options.integer("concurrency")
      .pipe(
        Options.withDescription(
          "Number of transaction batches to process simultaneously"
        )
      )
      .pipe(Options.withDefault(5)),
    ...
    interactive: Options.boolean("interactive")
      .pipe(
        Options.withDescription("Use interactive mode to configure options")
      )
    ...
    return pipe(
      Effect.tryPromise(async () => {
        try {
          ...
          if (interactive) {
            try {
              const { confirm, select, number, input } = await import(
                "@inquirer/prompts"
              );
              ...
              if (wallets.length === 0) {
                console.error(
                  chalk.red(
                    "❌ No wallets configured. Please add a wallet first."
                  )
                );
                console.log(
                  chalk.gray(
                    "Run: midgard-manager wallet add <n> --private-key <key>"
                  )
                );
                return;
              }
              ...
              if (config.transactionType === "mixed") {
                ...
                console.log(
                  chalk.dim(
                    `  ${chalk.cyan("ℹ")} This means ${
                      config.oneToOneRatio
                    }% simple and ${
              ...
              if (configureAdvanced) {
                ...
                console.log(
                  chalk.dim(
                    `  ${chalk.cyan(
                      "ℹ"
                    )} The endpoint where transactions will be submitted`
                  )
                );
              }
              ...
              const summaryTable = [
                ...
                [
                  ...
                  getTypeDescription(config.transactionType),
                ],
              ...
              summaryTable.forEach((row, index) => {
                if (index === 0) {
                  console.log(
                    chalk.cyan(
                      `  ${row[0].padEnd(12)}${row[1].padEnd(18)}${row[2]}`
                    )
                  );
              ...
              if (!confirm_start) {
                console.log(
                  chalk.yellow("Transaction generator setup cancelled.")
                );
                return;
              }
          ...
          try {
            ...
            console.log(
              `• Concurrency: ${config.concurrency} simultaneous batches`
            );
            ...
            console.log(
              chalk.dim(
                "• Press Ctrl+C to stop this process (generator will continue running)"
              )
            );
            ...
            if (
              ...
              console.log(
                chalk.gray("• Check that the Midgard node is running")
              );
              console.log(
                chalk.gray(`• Verify the node endpoint: ${config.nodeEndpoint}`)
              );
  ...
  Command.withDescription(
    "Generate and submit transactions to the Midgard node"
  )
);
...
export const txStatusCommand = Command.make("tx-status", {}, () => {
    ...
    Effect.tryPromise(async () => {
      ...
      try {
        ...
        if (getGeneratorStatus().running) {
          ...
          console.log(
            chalk.dim("• Start generator: midgard-manager generate-tx")
          );
        }
  ...
  Command.withDescription("Show the status of the transaction generator")
);
