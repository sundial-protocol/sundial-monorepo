  ...
  execute: async () => {
    try {
      ...
      const name = await input({
        ...
        validate: (value) =>
          value && value.trim() !== "" ? true : "Wallet name cannot be empty",
      });
  ...
  execute: async () => {
    try {
      ...
      if (availableWallets.length === 0) {
        console.log(
          chalk.yellow(
            "⚠️ Only the default test wallet exists, which cannot be removed"
          )
        );
        return {
          success: false,
      ...
      console.log(
        chalk.dim(
          "Remove a wallet from your configuration. This action cannot be undone.\n"
        )
      );
...
export const walletDetailsAction: Action = {
  ...
  execute: async () => {
    try {
      ...
      console.log();
      console.log(
        chalk.gray("This wallet can be used for transaction signing in:")
      );
      console.log(
        chalk.gray(
