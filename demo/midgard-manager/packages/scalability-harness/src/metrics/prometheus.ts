...
export const NODE_METRICS = [
  ...
  'tx_submissions_rejected_queue_backpressure_total{job="sundial_nodes",role=~"api|all"}',
  'tx_submissions_rejected_stream_backpressure_total{job="sundial_nodes",role=~"api|all"}',
  'tx_submissions_rejected_offer_timeout_total{job="sundial_nodes",role=~"api|all"}',
  'tx_submissions_mempool_accepted_total{job="sundial_nodes",role=~"tx-processor|all"}',
  'tx_stream_fail_total{job="sundial_nodes",role=~"tx-processor|all"}',
  'tx_stream_depth{job="sundial_nodes",role=~"tx-processor|all"}',
  'tx_stream_depth_peak{job="sundial_nodes",role=~"tx-processor|all"}',
  ...
  'submit_block_failures_total{job="sundial_nodes",role=~"sequencer|all"}',
  'submit_block_sign_timeouts_total{job="sundial_nodes",role=~"sequencer|all"}',
  'submit_block_submit_timeouts_total{job="sundial_nodes",role=~"sequencer|all"}',
  'submit_block_sign_duration_seconds_sum{job="sundial_nodes",role=~"sequencer|all"}',
  'submit_block_sign_duration_seconds_count{job="sundial_nodes",role=~"sequencer|all"}',
  'submit_block_submit_duration_seconds_sum{job="sundial_nodes",role=~"sequencer|all"}',
  'submit_block_submit_duration_seconds_count{job="sundial_nodes",role=~"sequencer|all"}',
  'commit_block_l1_user_events{job="sundial_nodes",role=~"sequencer|all"}',
  'commit_block_txs_per_block{job="sundial_nodes",role=~"sequencer|all"}',
  'commitment_window_tx_requests_total{job="sundial_nodes",role=~"sequencer|all"}',
  'commitment_window_tx_requests_selected{job="sundial_nodes",role=~"sequencer|all"}',
  'commitment_window_tx_requests_deferred{job="sundial_nodes",role=~"sequencer|all"}',
  'commitment_window_tx_requests_deferred_total{job="sundial_nodes",role=~"sequencer|all"}',
  'commit_block_events_size_bytes{job="sundial_nodes",role=~"sequencer|all"}',
  'commit_block_duration_seconds_sum{job="sundial_nodes",role=~"sequencer|all"}',
  'commit_block_duration_seconds_count{job="sundial_nodes",role=~"sequencer|all"}',
  'l1_commitment_fees_lovelace_total{job="sundial_nodes",role=~"sequencer|all"}',
  'l1_commitment_fee_lovelace_last{job="sundial_nodes",role=~"sequencer|all"}',
  'commit_block_commitment_failures_total{job="sundial_nodes",role=~"sequencer|all"}',
  'merge_block_count_total{job="sundial_nodes",role=~"sequencer|all"}',
  'merge_block_failures_total{job="sundial_nodes",role=~"sequencer|all"}',
