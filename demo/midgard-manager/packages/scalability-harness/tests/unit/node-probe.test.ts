...
describe('probeNode', () => {
  it('returns ok: true for a 404 response', async () => {
    const result = await probeNode('http://localhost:3000', 1000, makeFetcher(404));
    expect(result.ok).toBe(true);
    expect(result.statusCode).toBe(404);
  });

  it('returns ok: false for a 200 response', async () => {
  ...
  it('always includes a non-negative latencyMs', async () => {
    const ok = await probeNode('http://localhost:3000', 1000, makeFetcher(404));
  ...
  it('requests the correct probe URL with 64 zero hex chars', async () => {
    const fetcher = makeFetcher(404);
    await probeNode('http://localhost:3000', 1000, fetcher);
    const [url] = vi.mocked(fetcher).mock.calls[0];
    expect(url).toBe(`http://localhost:3000/tx?tx_hash=${'0'.repeat(64)}`);
  });
});
...
describe('runProbeLoop', () => {
  ...
  it('increments consecutive failures and resets on success', async () => {
    ...
    // After the 404 the counter must have been reset — the loop continued
