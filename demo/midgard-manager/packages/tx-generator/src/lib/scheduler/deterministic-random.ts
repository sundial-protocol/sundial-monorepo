...
/**
 * xfnv1a hash: deterministic 32-bit seed from text input.
 */
...
/**
 * mulberry32 PRNG: small deterministic RNG for reproducible workloads.
 */
...
export const createSeededRandom = (seed: string): (() => number) => {
  return createMulberry32(createSeedHash(seed));
};
